...
func getImg(url,name string) string{
    ...
    newUrl:=tempData[0]+"!600"

    picFile:=config.ImagePath+"experts/1/origin/"+name+"."+path[len(path)-1]
    ...
    return config.ImagePath+"experts/1/origin/"+name+"."+path[len(path)-1]
...
func resizeImg(imgDic,name string){
    ...
    file160,_:=os.Create(config.ImagePath+"experts/1/160/"+name+".jpg")
    defer file160.Close()
    file80,_:=os.Create(config.ImagePath+"experts/1/80/"+name+".jpg")
    defer file80.Close()
